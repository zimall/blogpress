...
function flot_activity(placeholder_id, user_options) {
	...
	var users = placeholder.data('users').data;
    var orders = placeholder.data('orders').data;
    var visitors = placeholder.data('visitors').data;
    var data_set = [];
        ...
        if(options.custom_tooltip) {
            if(item) {
                if(previousPoint != item.dataIndex) {
					...
					var m = date.getMonth();
					var d = date.getDay();
                    showTooltip(item.pageX, item.pageY, item.series.label + ": "+dayNames[d]+' '+n+' '+monthNames[m] + " = " + y);
                }
            } else {
                $("#tooltip").remove();
                previousPoint = null;
...
function flot_sales(placeholder_id, user_options) {
    var placeholder = $('#' + placeholder_id);
    var options = jQuery.extend({}, user_options);
    options.use_both = options.use_both === true ? true : false;
	...
	var sales = placeholder.data('sales').data;
    var rev = placeholder.data('revenue').data;
    var data_set = [];
...
function flot_line(placeholder_id, user_options, data_set, labels)
{
	...
	if( !$(placeholder).data('users') )
	{
		alert('no data found for graph '+placeholder);
		return;
	}
    ...
    if(data_set === null || data_set === undefined){
        data_set = [ users, orders, visitors ];
    }

    if(labels === null || labels === undefined){
        labels = ["Users", "Orders", "Visitors"];
    }
...
function flot_line_b(placeholder_id, user_options, data_set, labels) {
    var placeholder = $('#' + placeholder_id);
    var options = jQuery.extend({}, user_options);
