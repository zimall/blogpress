...
class Account extends CI_Controller
{
	...
	public function login()
	{
		...
		{
			sem('You are already logged in.',2);
			redirect('home');
		}
	...
	public function index()
	{
		...
		$this->load->view( "account/account.tpl", $this->data );
	}
	...
	public function register()
	{
		...
		elseif($this->input->post('register_user'))
		{			
			$this->load->model('auth_model');
	...
	function resend_activation_token()
	{
		...
		$this->data['section'] = 'resend_activation_token';
		$this->load->view( "account/account.tpl", $this->data );
	}
	...
	function lost_password()
	{
		...
		// Get any status message that may have been set.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
		$this->data['section'] = 'lost_password';
		$this->load->view( "account/account.tpl", $this->data );	
	}
	...
	function manual_reset_forgotten_password($user_id = FALSE, $token = FALSE)
	{
		...
		// Get any status message that may have been set.
		$this->data['message'] = (! isset($this->data['message'])) ? $this->session->flashdata('message') : $this->data['message'];		
		$this->data['section'] = 'forgot_password_update_view';
		$this->load->view( "account/account.tpl", $this->data );
	}
