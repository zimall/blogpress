...
class Auth_model extends CI_Model
{
	...
	function login()
	{
		...
		if ($this->form_validation->run())
		{
			...
			// Verify login data.
			$login = $this->flexi_auth->login($this->input->post('login_identity'), $this->input->post('login_password'), $remember_user);
			...
			if($login)
			{
				$r = $this->authorize->get_return_url();
				if(!$r) $r = 'home';
				redirect($r);
			}
			else return $login;
	 ...
	 */
	function register_account($first=FALSE)
	{
		$this->load->library('form_validation');
		...
		if ($this->form_validation->run())
		{
			...
			$profile_data = array(
				...
				'newsletter' => $this->input->post('register_newsletter')
			);
			...
			$response = $this->flexi_auth->insert_user($email, $username, $password, $profile_data, $group, $instant_activate, $first);
			...
			if ($response)
			{
				...
				{
					if( $first ) redirect('account/post_installation_cleanup');
				}
	...
	function resend_activation_token()
	{
		...
		{					
			...
			// Redirect user.
			($response) ? redirect('account/login') : redirect('account/resend_activation_token');
		}
		else
		{	
			// Set validation errors.
