...
class Auth_admin_model extends CI_Model
{
	...
	function update_user_account($user_id)
	{
		...
		$validation_rules = array(
			...
			array('field' => 'update_phone', 'label' => 'Phone Number', 'rules' => 'required'),
			array('field' => 'update_newsletter', 'label' => 'Newsletter', 'rules' => 'integer'),
			array('field' => 'update_email_address', 'label' => 'Email Address', 
				'rules' => 'required|valid_email|identity_available['.$user_id.']'),
			array('field' => 'update_username', 'label' => 'Username', 'rules' => 'min_length[4]|identity_available['.$user_id.']'),
			array('field' => 'update_user_group', 'label' => 'User Group', 'rules' => 'required|integer')
		);
		...
		if ($this->form_validation->run())
		{
			...
			$profile_data = array(
				...
				'phone' => $this->input->post('update_phone_number'),
				'newsletter' => $this->input->post('update_newsletter'),
				'bio' => htmlentities( $this->input->post('update_bio') ),
				...
				$this->flexi_auth->db_column('user_acc', 'group_id') => $this->input->post('update_user_group')
			);
			...
			return $this->flexi_auth->update_user($user_id, $profile_data);
